...
- [ ] Реализовать In-Memory Cache для хранения состояния пользователя
- [ ] Создать интерфейс `IBotCommand` с методом `ExecuteAsync`
- [ ] Реализовать базовые команды MVP:
...
- [ ] Создать Command Router для маршрутизации команд
- [ ] Настроить обработку ошибок и логирование

## 📊 Фаза 7: Статистика и отчеты
- [ ] Реализовать `IStatisticsService`
- [ ] Реализовать команду `/stats` с выбором периода
- [ ] Генерация отчетов:
  - За день
  - За неделю
  - За месяц
- [ ] Форматирование текста для красивого вывода в Telegram

## ✏️ Фаза 8: Редактирование и удаление
- [ ] Команда `/edit` - редактирование привычки (название, время, частота)
- [ ] Команда `/delete` - удаление привычки (мягкое удаление)
- [ ] Inline клавиатуры для выбора привычки

## 🔒 Фаза 9: Безопасность и производительность
- [ ] Реализовать Rate Limiting (защита от спама)
- [ ] Sanitize пользовательских данных перед отправкой
- [ ] Добавить correlation ID для трейсинга запросов
- [ ] Настроить Serilog с structured logging
- [ ] Настроить разные log levels (Debug/Information)

## 🧪 Фаза 10: Тестирование
- [ ] Установить NuGet: xUnit, Moq, FluentAssertions
- [ ] Unit тесты для сервисов (HabitService, UserService)
- [ ] Unit тесты для команд бота
- [ ] Integration тесты для репозиториев (InMemory DB)
- [ ] Тестирование валидаторов
- [ ] Покрытие критичной логики тестами >80%
