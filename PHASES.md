...
- [x] Установить NuGet: Quartz, Quartz.Extensions.Hosting
- [x] Настроить Quartz.NET в DI контейнере
- [x] Создать Job для отправки напоминаний (`ReminderJob`)
- [x] Настроить динамические триггеры по времени пользователя (запуск каждую минуту)
- [x] Обработка временных зон (TimeZoneInfo) через `TimeZoneParser`
- [x] Inline-кнопка «✅ Выполнено!» прямо в напоминании для быстрой отметки
