...
### 📈 Ежедневная сводка
- [ ] Установить NuGet: Quartz, Quartz.Extensions.Hosting (если еще не установлен)
- [ ] Добавить поле `DailySummaryTime` (nullable TimeSpan) и `IsDailySummaryEnabled` (bool) к сущности User
- [ ] Создать миграцию для добавления полей сводки
- [ ] Реализовать `IDailySummaryService` для генерации сводки:
...
- [ ] Реализовать команду `/setsummary` для настройки времени и включения/отключения сводки
- [ ] Реализовать команду `/summary` для получения сводки в любое время
- [ ] Создать Quartz Job для отправки ежедневной сводки:
  - Динамические триггеры по времени каждого пользователя (с учетом временной зоны)
  - Обработка включения/отключения сводки
- [ ] Добавить уведомление после добавления/изменения привычки о настройке сводки
- [ ] Тестирование сводки и Job
