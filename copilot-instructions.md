...
- PostgreSQL (база данных). База данных будет установлена в docker-контейнере.
...
### Ежедневная сводка
- Реализовать ежедневную сводку по привычкам пользователя.
- Пользователь может выбрать время получения сводки.
- Сводка включает статистику по выполнению привычек за день.
- Если есть выполненные привычки, то в сводке для неё указывается время выполнения. Если привычка не выполнена, то указывается "не выполнено". Если привычка не была запланирована на этот день, то она не включается в сводку.
- В сводке также может быть указано общее количество выполненных привычек за день и процент выполнения от запланированных привычек. Например, "Сегодня ты выполнил 3 из 5 привычек (60%)".
- В заключительно части сводки приводится список привычек, запланированных на завтра. Например, "Завтра у тебя запланированы следующие привычки: Медитировать в 08:00, Чтение в 20:00".
- По умолчанию сводка отправляется в 21:00, но пользователь может изменить время получения сводки через команду /setsummary.
- Для реализации ежедневной сводки использовать Quartz.NET для планирования задач и Cron-выражения для настройки времени отправки сводки. Учитывать временные зоны пользователей при отправке сводки.
- Сводку можно отключать через команду /setsummary.
- После каждого добавления или изменения привычки присылать пользователю сообщение, например, "Я буду присылать тебе сводку о выполнении привычек ежедневно в 21:00. Это поведение можно изменить или отключить через команду /setsummary". Если пользователь уже настроил время получения сводки, то после изменения привычки присылать сообщение, например, "Я буду присылать тебе сводку о выполнении привычек ежедневно в 08:00. Это поведение можно изменить или отключить через команду /setsummary". Если пользователь отключил сводку, то после изменения привычки присылать сообщение, например, "Сводка о выполнении привычек отключена. Ты всегда можешь включить её через команду /setsummary".
- Сводку можно посмотреть у любое время через команду /summary. В этом случае сводка будет сгенерирована на момент запроса, а не по расписанию. Если пользователь отключил сводку, то по команде /summary сводка всё равно отображается. При этом сводка содержит информацию о том, какие ещё привычки можно успеть выполнить сегодня из запланированных, план на следующий день не отображается. 

### Напоминания
- Реализация напоминаний в последнюю очередь, после реализации основной функциональности.
